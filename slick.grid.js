/**
 * @license (c) 2009-2016 Michael Leibman michael{dot}leibman{at}gmail{dot}com http://github.com/mleibman/slickgrid
 * 
 * Distributed under MIT license. All rights reserved.
 * 
 * SlickGrid v2.3
 * 
 * NOTES: Cell/row DOM manipulations are done directly bypassing jQuery's DOM manipulation methods. This increases the speed dramatically, but can
 * only be done safely because there are no event handlers or data associated with any cell/row DOM nodes. Cell editors must make sure they implement
...
(function ($) {
	...
	function SlickGrid(container, data, columns, options) {
		...
		var defaults = {
			...
			rightColumnFrozen: null,
			leftColumnFrozen: null
		};
		...
		var totalWidth, canvasWidth, canvasWidthL, canvasWidthR;
		...
		function init() {
			...
			if (options.leftColumnFrozen) {
				...
				$headerScrollerL = $("<div class='slick-header ui-state-default slick-header-left' style='overflow:hidden;position:relative;' />").appendTo($paneHeaderL);
			...
			if (options.rightColumnFrozen || options.leftColumnFrozen) {
				$paneHeader = $("<div class='slick-pane slick-pane-header' tabIndex='0' />").appendTo($container);
				$headerScroller.appendTo($paneHeader);
			} else {
				$headerScroller.appendTo($container);
			}
			...
			if (options.rightColumnFrozen) {
				...
				$headerScrollerR = $("<div class='slick-header ui-state-default slick-header-right' style='overflow:hidden;position:relative;' />").appendTo($paneHeaderR);
				$headersR = $("<div class='slick-header-columns 'slick-header-columns-right' style='left:-1000px' />").appendTo($headerScrollerR);
			}

			setHeadersWidth(); // $headers.width(getHeadersWidth());
			...
			$headerRowSpacer = $("<div style='display:block;height:1px;position:absolute;top:0;left:0;'></div>").css("width",
				getCanvasWidth().totalWidth + scrollbarDimensions.width + "px").appendTo($headerRowScroller);

			$topPanelScroller = $("<div class='slick-top-panel-scroller ui-state-default' style='overflow:hidden;position:relative;' />").appendTo(
				$container);
			...
			if (options.leftColumnFrozen) {
				$paneViewportL = $("<div class='slick-pane slick-pane-left' tabIndex='0' />").appendTo($container);
				$viewportL = $("<div class='slick-viewport slick-viewport-left' style='width:100%;overflow:auto;outline:0;position:relative;;'>").appendTo($paneViewportL);
				$viewportL.css("overflow-y", options.autoHeight ? "hidden" : "auto");

				$canvasL = $("<div class='grid-canvas' />").appendTo($viewportL);
			}
			if (options.leftColumnFrozen || options.rightColumnFrozen) {
				$paneViewport = $("<div class='slick-pane slick-pane' tabIndex='0' />").appendTo($container);
				$viewport = $("<div class='slick-viewport' style='width:100%;overflow:auto;outline:0;position:relative;;'>").appendTo($paneViewport);
			...
			if (options.createFooterRow) {
				...
				$footerRowSpacer = $("<div style='display:block;height:1px;position:absolute;top:0;left:0;'></div>").css("width",
					getCanvasWidth().totalWidth + scrollbarDimensions.width + "px").appendTo($footerRowScroller);
		...
		function finishInitialization() {
			if (!initialized) {
				...
				if (options.leftColumnFrozen) {
					$viewportL.on("scroll", handleScroll);
				...
				if (options.leftColumnFrozen) {
					$headerScrollerL.on("click", handleHeaderClick);
				}
				$headerScroller.on("contextmenu", handleHeaderContextMenu).on("click", handleHeaderClick).on("mouseenter", ".slick-header-column",
					handleHeaderMouseEnter).on("mouseleave", ".slick-header-column", handleHeaderMouseLeave);
				...
				$canvas.on("keydown", handleKeyDown).on("click", handleClick).on("dblclick", handleDblClick).on("contextmenu", handleContextMenu).on(
					...
					"mouseleave", ".slick-cell", handleMouseLeave);
				...
				if (navigator.userAgent.toLowerCase().match(/webkit/) && navigator.userAgent.toLowerCase().match(/macintosh/)) {
					...
					if (options.leftColumnFrozen) {
						$canvasL.on("mousewheel", handleMouseWheel);
					}
				}
		...
		function measureScrollbar() {
			var $c = $("<div style='position:absolute; top:-10000px; left:-10000px; width:100px; height:100px; overflow:scroll;'></div>").appendTo(
				"body");
			var dim = {
		...
		function setHeadersWidth() {
			var headersWidth = headersWidthL = headersWidthR = 0;

			for (var i = 0, colCount = columns.length; i < colCount; i++) {
				var width = columns[i].width;

				if (options.leftColumnFrozen && (i < options.leftColumnFrozen)) {
					headersWidthL += width;
					continue;
				}

				if (options.rightColumnFrozen && (i >= (colCount - options.rightColumnFrozen))) {
					headersWidthR += width;
					continue;
				}
			...
			if (options.leftColumnFrozen) {
				...
				if (options.rightColumnFrozen) {
					headersWidthR = headersWidthR + 1000 + scrollbarDimensions.width;
					$headersR.width(headersWidthR);

					headersWidth = headersWidth + 1000;
					$headers.width(headersWidth);
				} else {
					headersWidth += scrollbarDimensions.width;
					headersWidth = Math.max(headersWidth, viewportW) + headersWidthL + 1000;
		...
		function getCanvasWidth() {
			...
			while (i--) {
				...
				canvasWidth += columns[i].width;
			}

			var totalRowWidth = canvasWidthL + canvasWidthR + canvasWidth;
			return {
				'totalWidth': options.fullWidthRows ? Math.max(totalRowWidth, availableWidth) : totalRowWidth,
		...
		function updateCanvasWidth(forceColumnWidthsUpdate) {
			var oldTotalWidth = totalWidth,
				oldCanvasWidth = canvasWidth,
				oldCanvasWidthL = canvasWidthL,
				oldCanvasWidthR = canvasWidthR;
			var oldWidth = getCanvasWidth();

			totalWidth = oldWidth.totalWidth, canvasWidth = oldWidth.canvasWidth, canvasWidthL = oldWidth.canvasWidthL, canvasWidthR = oldWidth.canvasWidthR;
			var widthChanged = (oldTotalWidth !== totalWidth || canvasWidth !== oldCanvasWidth || canvasWidthL !== oldCanvasWidthL || canvasWidthR !== oldCanvasWidthR);
			if (widthChanged) {
				if (options.leftColumnFrozen) {
					$paneViewportL.width(canvasWidthL);
					$canvasL.width(canvasWidthL);

					if (options.rightColumnFrozen) {
						$paneHeader.css('left', canvasWidthL)
						$paneHeader.width(viewportW - (canvasWidthL + canvasWidthR) - scrollbarDimensions.width);
						$paneViewport.css('left', canvasWidthL);
						$paneViewport.width(viewportW - (canvasWidthL + canvasWidthR) - scrollbarDimensions.width);
						...
						$paneViewport.width(viewportW - canvasWidthL);
						$canvas.width(canvasWidth);
					...
					if (options.rightColumnFrozen) {
						...
						$viewport.width(viewportW - canvasWidthR - scrollbarDimensions.width);
						$canvas.width(canvasWidth);
				...
				if (options.createFooterRow) {
					$footerRow.width(totalWidth);
				}
				setHeadersWidth(); // $headers.width(getHeadersWidth());

				viewportHasHScroll = (totalWidth > viewportW - scrollbarDimensions.width);
			}

			var w = totalWidth + (viewportHasVScroll ? scrollbarDimensions.width : 0);
		...
		function createColumnHeaders() {
			...
			for (var i = 0, colCount = columns.length; i < colCount; i++) {
				...
				if (options.leftColumnFrozen && i < options.leftColumnFrozen) {
					$headerTarget = $headersL;
				} else if (options.rightColumnFrozen && i >= (colCount - options.rightColumnFrozen)) {
					$headerTarget = $headersR;
				} else {
					$headerTarget = $headers;
				}

				var header = $("<div class='ui-state-default slick-header-column' />").html("<span class='slick-column-name'>" + m.name + "</span>")
					.width(m.width - headerColumnWidthDiff).attr("id", "" + uid + m.id).attr("title", m.toolTip || "").data("column", m)
					.addClass(m.headerCssClass || "").appendTo($headerTarget);
				...
				if (columns[i].filterable) {
					if (options.showHeaderRow) {
						var headerRowCell = $("<div class='ui-state-default slick-headerrow-column l" + i + " r" + i + "'></div>").data("column", m)
							.appendTo($headerRow);
				...
				if (options.createFooterRow && options.showFooterRow) {
					var footerRowCell = $("<div class='ui-state-default slick-footerrow-column l" + i + " r" + i + "'></div>").data("column", m)
						.appendTo($footerRow);
		...
		function setupColumnSort() {
			var headerSet = $();
			if (options.leftColumnFrozen) {
				headerSet = headerSet.add($headersL);
			}
			headerSet = headerSet.add($headers);
			headerSet.click(function (e) {
		...
		function setupColumnReorder() {
			...
			if (options.leftColumnFrozen) {
				$headersL.filter(":ui-sortable").sortable("destroy");
				$headersL.sortable({
					containment: "parent",
					distance: 3,
					axis: "x",
					...
					stop: function (e) {
						...
						var reorderedIds = $headersL.sortable("toArray");
		...
		function setupColumnResize() {
			...
			var $$headers = $();
			if (options.leftColumnFrozen) {
				$$headers = $$headers.add($headersL);
			}
			$$headers = $$headers.add($headers);
			if (options.rightColumnFrozen) {
				$$headers = $$headers.add($headersR);
			}
		...
		function _setupColumnResize() {
			...
			var gap = 0;
			if (options.leftColumnFrozen) {
				gap = options.leftColumnFrozen;
			}
			columnElements = $headers.children();
			columnElements.find(".slick-resizable-handle").remove();
			columnElements.each(function (i, e) {
				if (columns[i + gap].resizable) {
					if (firstResizable === undefined) {
						firstResizable = i;
					}
					lastResizable = i;
			...
			columnElements.each(function (i, e) {
				...
				$("<div class='slick-resizable-handle' />").appendTo(e).on("dragstart", function (e, dd) {
					...
					// lock each column's width option to current width
					columnElements.each(function (i, e) {
						columns[i + gap].previousWidth = $(e).outerWidth();
					});
					if (options.forceFitColumns) {
						shrinkLeewayOnRight = 0;
						stretchLeewayOnRight = 0;
		...
		function measureCellPaddingAndBorder() {
			...
			// jquery 1.8 changed .width to read the true inner element width if box-sizing is set to border-box, and introduced a setter for
			// .outerWidth
		...
		function createCssRules() {
			...
			var rules = ["." + uid + " .slick-header-column { left: 1000px; }",
				...
				"." + uid + " .slick-cell { height:" + rowHeight + "px; }", "." + uid + " .slick-row { height:" + options.rowHeight + "px; }"
			];
		...
		function autosizeColumns() {
			var i, c, widths = [],
				shrinkLeeway = 0,
				total = 0,
				prevTotal, availWidth = viewportHasVScroll ? viewportW - scrollbarDimensions.width :
				viewportW;
		...
		function applyColumnWidths() {
			...
			for (var i = 0, colCount = columns.length; i < colCount; i++) {
				...
				if (options.leftColumnFrozen && i < options.leftColumnFrozen) {
					rule.right.style.right = (canvasWidthL - x - w) + "px";
				} else if (options.rightColumnFrozen && i >= (colCount - options.rightColumnFrozen)) {
					rule.right.style.right = (canvasWidthR - x - w) + "px";
				} else {
		...
		function setSortColumns(cols) {
			...
			headerSet = headerSet.add($headers);
			var headerColumnEls = headerSet.children();
		...
		function getFormatter(row, column) {
			...
			return (columnOverrides && columnOverrides.formatter) || (rowMetadata && rowMetadata.formatter) || column.formatter ||
				(options.formatterFactory && options.formatterFactory.getFormatter(column)) || options.defaultFormatter;
		}
		...
		function appendRowHtml(stringArrayL, stringArray, stringArrayR, row, range, dataLength) {
			...
			for (var i = 0, ii = columns.length; i < ii; i++) {
				...
				if (columnPosRight[Math.min(ii - 1, i + colspan - 1)] > range.leftPx) {
					...
					} else if (options.rightColumnFrozen && i >= (ii - options.rightColumnFrozen)) {
		...
		function appendCellHtml(stringArray, row, cell, colspan, item) {
			...
			if (item) {
				...
				formatterContent = getFormatter(row, m)(row, cell, value, m, item)
			}

			if (formatterContent) {
				if ('string' == typeof (formatterContent) || 'number' == typeof (formatterContent)) {
					stringArray.push("<div class='" + cellCss + "'>");
					stringArray.push(formatterContent);
				} else {
					if (formatterContent.cellCssClass) {
						stringArray.push("<div class='" + cellCss + ' ' + formatterContent.cellCssClass + "'>");
					} else {
						stringArray.push("<div class='" + cellCss + "'>");
					}

					stringArray.push(formatterContent.content);
				}
			} else {
				stringArray.push("<div class='" + cellCss + "'>");
			}
			...
			if (rowNodeFromLastMouseWheelEvent === cacheEntry.rowNode) {
				...
				if (options.enableAsyncPostRenderCleanup && postProcessedRows[row]) {
					...
					if (options.leftColumnFrozen) {
						...
						if (options.rightColumnFrozen) {
							cacheEntry.rowNode[2].parentElement.removeChild(cacheEntry.rowNode[2]);
						}
					} else {
						if (options.rightColumnFrozen) {
							cacheEntry.rowNode[0].parentElement.removeChild(cacheEntry.rowNode[0]);
		...
		function updateCell(row, cell) {
			...
			if (currentEditor && activeRow === row && activeCell === cell) {
				...
				if (d) {
					var formatterContent = getFormatter(row, m)(row, cell, getDataItemValueForColumn(d, m), m, d);
					if ('string' == typeof (formatterContent) || 'number' == typeof (formatterContent)) {
						cellNode.innerHTML = formatterContent;
					} else {
						if (formatterContent.cellCssClass) {
							$(cellNode).addClass(formatterContent.cellCssClass);
						}
						cellNode.innerHTML = formatterContent.content;
					}
				} else {
					cellNode.innerHTML = "";
				}
		...
		function updateRow(row) {
			...
			for (var columnIdx in cacheEntry.cellNodesByColumnIdx) {
				...
				if (row === activeRow && columnIdx === activeCell && currentEditor) {
					...
					var formatterContent = getFormatter(row, m)(row, columnIdx, getDataItemValueForColumn(d, m), m, d);
					if ('string' == typeof (formatterContent) || 'number' == typeof (formatterContent)) {
						node.innerHTML = formatterContent;
					} else {
						if (formatterContent.cellCssClass) {
							$(node).addClass(formatterContent.cellCssClass);
						}
						node.innerHTML = formatterContent.content;
					}
				} else {
					node.innerHTML = "";
				}
		...
		function getViewportHeight() {
			return parseFloat($.css($container[0], "height", true)) - parseFloat($.css($container[0], "paddingTop", true)) -
		...
		function resizeCanvas() {
			...
			if (!options.autoHeight) {
				...
				if (options.leftColumnFrozen) {
					$viewportL.height(viewportH);
				}

				if (options.rightColumnFrozen) {
					$viewportR.height(viewportH);
				}
			}
			...
			if (options.rightColumnFrozen || options.leftColumnFrozen) {
				...
				if (options.rightColumnFrozen) {
					$paneViewportR.css('top', $paneHeaderR.height());
				}
			}
		...
		function updateRowCount() {
			...
			if (h !== oldH) {
				if (options.leftColumnFrozen) {
					$canvasL.css("height", h);
				}
				$canvas.css("height", h);
				scrollTop = $viewport[0].scrollTop;
				if (options.rightColumnFrozen) {
					$canvasR.css("height", h);
				}
			}
		...
		function ensureCellNodesInRowsCache(row) {
			...
			if (cacheEntry) {
				if (cacheEntry.cellRenderQueue.length) {
					var cells = [];
					cacheEntry.rowNode.each(function (idx, element) {
						element.childNodes.forEach(function (subElement, subIdx) {
							cells.push(subElement);
						});
					});
					while (cacheEntry.cellRenderQueue.length) {
						var columnIdx = cacheEntry.cellRenderQueue.pop();
						cacheEntry.cellNodesByColumnIdx[columnIdx] = cells[columnIdx];
					}
				}
			}
		}
		...
		function renderRows(range) {
			...
			for (var i = range.top, ii = range.bottom; i <= ii; i++) {
				...
				appendRowHtml(stringArrayL, stringArray, stringArrayR, i, range, dataLength);
			...
			x.innerHTML = stringArray.join("");
			xL.innerHTML = stringArrayL.join("");
			xR.innerHTML = stringArrayR.join("");

			for (var i = 0, ii = rows.length; i < ii; i++) {
				var cacheNode = $();
				if (options.leftColumnFrozen) {
					cacheNode = cacheNode.add($(xL.firstChild).appendTo($canvasL));
				}
				cacheNode = cacheNode.add($(x.firstChild).appendTo($canvas));
				if (options.rightColumnFrozen) {
					cacheNode = cacheNode.add($(xR.firstChild).appendTo($canvasR));
				}

				rowsCache[rows[i]].rowNode = cacheNode;
			}
		...
		function _handleMouseWheel(event, delta, deltaX, deltaY) {
			scrollTop = Math.max(0, $viewport[0].scrollTop - (deltaY * options.rowHeight));
			handleScroll(event, true);
			event.preventDefault();
		}

		function handleScroll(event, isMouseWheel) {
			if (isMouseWheel) {} else {
		...
		function makeActiveCellNormal() {
			...
			if (activeCellNode) {
				...
				if (d) {
					...
					var formatterContent = formatter(activeRow, activeCell, getDataItemValueForColumn(d, column), column, d, self);
					if ('string' == typeof (formatterContent) || 'number' == typeof (formatterContent)) {
						activeCellNode.innerHTML = formatterContent;
					} else {
						if (formatterContent.cellCssClass) {
							$(activeCellNode).addClass(formatterContent.cellCssClass);
						}
						activeCellNode.innerHTML = formatterContent.content;
					}
